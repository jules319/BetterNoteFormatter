...
class PDFProcessorController:
    ...
    def files_or_folders_on_drop(self, event):
        ...
        for full_path in paths:
            if (
                full_path.endswith(".pdf")
                and os.path.isfile(full_path)
                and full_path not in self.full_paths
            ):  # If it's a file, add directly
                self._add_file_to_queue(full_path)
            elif os.path.isdir(
                full_path
            ):  # If it's a directory, add all PDF files in it
    ...
    def browse_output_folder(self):
        ...
        if output_folder_path:
            ...
            self.view.output_entry.delete(
                0, tk.END
            )  # Clear the existing text in the Entry
            self.view.output_entry.insert(
                0, output_folder_path
    ...
    def process_files(self):
        ...
        output_folder = (
            self.view.output_entry.get()
        )  # TODO: Check output folder is valid
